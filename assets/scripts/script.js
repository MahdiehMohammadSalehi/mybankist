...
const displayMovments = (acc, sort = false) => {
    ...
    mov.forEach(function (mov, i) {
        ...
        if (i % 2 == 0)
            document.querySelector('.movements__row').style.backgroundColor = '#cfc7c7';
...
const displaySummary = function (acc) {
    summaryin.textContent = formatCur(
        acc.movements
            ...
            .toFixed(2), acc.locale, acc.currency);
...
const checkUserPIN = function () {
    ...
    if (curUser) {
        ...
        currentDate.textContent = new Intl.DateTimeFormat(browserLocale, options).format(now);
